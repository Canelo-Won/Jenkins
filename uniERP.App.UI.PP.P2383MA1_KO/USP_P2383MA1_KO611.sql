USE [bixolon]
GO
/****** Object:  StoredProcedure [dbo].[USP_P2383MA1_KO611]    Script Date: 2022-01-17 오후 1:38:32 ******/
SET ANSI_NULLS ON
GO
...
/*--------------------------------------------------------------------------------------------
--개요 : 전용자재 전개하는 로직
--SP명 : USP_P2383MA1_KO611
--관련SP : USP_P2383MA1_QUERY_KO611 (BOM 기준으로 전개를 하고 USER 기준으로 전개를 하여 p_temp_lim_2에 저장 후 조건에 맞게 조회
...
-- IF (@ITEM_GRP1 <> '%' AND @ITEM_GRP2 <> '%' AND @ITEM_CD <> '%' AND @tracking_no = '%') -- 제품분류  
--  BEGIN    
--   SELECT DISTINCT OSD.PLANT_CD   
--    ,X.대그룹_코드  AS ITEM_GRP1    
--    ,X.대그룹_명칭 AS ITEM_GRP1_NM    
